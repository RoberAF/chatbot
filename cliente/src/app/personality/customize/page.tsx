...
export default function PersonalityCustomizePage() {
  ...
  // Cargar la plantilla desde sessionStorage o desde el ID de la URL
  useEffect(() => {
    ...
    // Si no hay en sessionStorage, buscar por ID de URL
    if (templateId) {
      ...
      // Si no hay templateId, redirigir a la selección
      router.push('/personality/new');
    }
  }, [templateId, router]);

  // Función para llenar el formulario con los datos de la plantilla
  const populateFormFromTemplate = (template: PersonalityTemplate) => {
    ...
    // Campos opcionales según el tipo de plantilla
  ...
  // Generar una vista previa de cómo respondería esta personalidad
  const generatePreview = () => {
    setPreview('Generando vista previa...');
    
    // Ejemplo de conversación para mostrar la personalidad
    // En un caso real, esto podría usar una llamada a la API
    setTimeout(() => {
      let previewText = '';
  ...
  // Guardar la personalidad
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setError(null);

    // Convertir campos string a arrays donde sea necesario
    ...
    try {
      // Llamar a la API para guardar la personalidad
      ...
      // Seleccionar la personalidad recién creada
      await authFetch(`/users/me/personalities/${id}/select`, {
        method: 'POST',
      });

      // Redirigir al chat con la nueva personalidad
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 py-12">
      <div className="max-w-3xl mx-auto px-4">
        ...
        {/* Mensaje de error */}
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-6 dark:bg-red-900/30 dark:border-red-800 dark:text-red-300">
            {error}
          </div>
        )}
