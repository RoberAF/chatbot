// src/app/personality/setup/page.tsx
'use client';
...
export default function SetupPage() {
  ...
  // Ya no forzamos solo 'PREMIUM', dejamos que sea cualquiera de:
  ...
  // 1) Esperamos a que termine de cargar auth
  useEffect(() => {
  ...
  // 3) Una vez authLoading===false y loading===false,
  //    solo **PRO** y **PRO_PLUS** pueden seguir aquí.
  useEffect(() => {
    if (authLoading || loading) return;

    // si no somos PRO ni PRO_PLUS → volvemos al chat
    if (tier !== 'PRO' && tier !== 'PRO_PLUS') {
      router.push('/chat');
    }
  }, [authLoading, loading, tier, router]);

  // 4) Render mientras carga o si hay error
